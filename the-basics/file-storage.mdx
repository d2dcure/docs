...
- You would send a POST call to this endpoint to upload a file to the bucket
- You would send a GET call to this endpoint to retrieve a file from the bucket
- You would send a DELETE call to this endpoint to delete a file from the bucket
- Notice how you also need to pass a folder name as well to this endpoint: this is because any file you want to upload/get/delete must have a folder associated with it, files do not live directly in the bucket, they live in folders in the bucket
...
3. **API endpoint receives** the file data with POST
...
For an example of the uploading and fetching files from S3 in action, explore the `components/GelUploadedView.tsx` file\! 
